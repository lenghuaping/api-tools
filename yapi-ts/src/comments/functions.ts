import { IApiresData } from "../interfaces/apires";
...
function getFunctionHeader(
  ...
  const fnName = funcName.indexOf('?') !== -1 ? funcName.slice(0, funcName.indexOf('?') - 1) : funcName
  let newFuncName = toUpperCaseFirst(fnName)
  ...
  return `export async function ${fnName}(${querList.join(', ')}) {\n`
}
  ...
  if (type === 'object') {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (type === 'object') {
        ...
        }: any = dpCreateInterface(nextName, properties[key], interfaces, index + 1)
        const list = new Array(Number(max) - index).fill(1)
        maxIndex = max
        str += `  ${key}: ${newName}${list.reduce(pre => pre + '[]', '')};`
      } else {
        str += `  ${key}: ${type === 'integer' || type === 'Long' ? 'number' : type};`
        isBasic = true
        interfaceName = type
      }
      if (description) {
